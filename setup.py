...
template_files = [os.path.relpath(f'templates/{f}', 'templates') for f in glob('templates/**', recursive=True) if os.path.isfile(f)]
...
setup(
    ...
    entry_points={
        'console_scripts': [
            ...
            'tradingbot_metrics = kiteAPI:pnlmetrics'
        ]
    },
)
